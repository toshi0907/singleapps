...
拡張子ではなく完全一致のファイル名で対象を絞りたい場合は、`filenames` 配列を使用します。

```json
"filenames": ["test.mylang"]
```

`filenames` と `extensions` は併用可能です。glob パターンで指定したい場合は `filenamePatterns` を使用します。

```json
"filenamePatterns": ["*.test.mylang"]
```
